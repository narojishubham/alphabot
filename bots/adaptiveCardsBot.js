  ...
  constructor() {
    ...
    this.onMessage(async (context, next) => {
        ...
        switch (context.activity.value.card) {
          ...
          case "Confirm":
            ...
            if (
              ...
              offerDetails.comments.trim().length === 0
            ) {
              offerDetails.comments = "N/A";
            }
            offerDetails.salary = context.activity.value.salary;
            offerDetails.startDate = moment(
              context.activity.value.startDate,
              "YYYY-MM-DD"
            ).format("DD MMMM YYYY");
            // await context.sendActivity({
            ...
            await context.sendActivity({
              attachments: [
                CardFactory.adaptiveCard(offerSummarySchema),
                CardFactory.adaptiveCard(OfferSentNotification),
              ],
            });
            await context.sendActivity({
              text: "Sarah signed the offer",
      ...
      if (
        ...
        context.activity.text.trim().toLowerCase() === "show candidates"
      ) {
        await context.sendActivity({
